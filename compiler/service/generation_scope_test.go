...
`
	ignoredPaths := []string{
		...
		"internal/processcontainment/internal/spicegen/generated/doc.go",
	}
	for _, relative := range ignoredPaths {
	...
	testPath := filepath.Join(root, "internal", "processcontainment", "tests", "ignored_test.go")
	oversizedPath := filepath.Join(root, "internal", "processcontainment", "oversized", "doc.go")
	smallPhysicalPath := filepath.Join(root, "internal", "processcontainment", "smallphysical", "doc.go")
	largePhysicalPath := filepath.Join(root, "internal", "processcontainment", "largephysical", "doc.go")
	for _, directory := range []string{filepath.Dir(testPath), filepath.Dir(oversizedPath)} {
	...
	writeServiceFixtureFile(
		t,
		root,
		"internal/processcontainment/smallphysical/doc.go",
		"package smallphysical\n",
	)
	largePhysical := append(
		[]byte("package largephysical\n// "),
		make([]byte, generationInventoryMaximumFileBytes)...,
	)
	writeServiceFixtureFile(
		t,
		root,
		"internal/processcontainment/largephysical/doc.go",
		string(largePhysical),
	)
	linkedPath := ""
	linkedTarget := filepath.Join(root, "internal", "processcontainment", "linked-target")
	if err := os.MkdirAll(filepath.Join(linkedTarget, "ignored"), 0o750); err != nil {
		t.Fatal(err)
	}
	link := filepath.Join(root, "internal", "processcontainment", "linked")
	if err := os.Symlink(linkedTarget, link); err == nil {
		linkedPath = filepath.Join(link, "ignored", "doc.go")
	}
	overlay := map[string]Document{
		interfacePath: {Version: 7, Content: []byte(interfaceSource)},
		generatedPath: {
			Version: 7,
			Content: []byte("// Code generated by Spice. DO NOT EDIT.\n\n" + ignoredInterface),
		},
		testPath: {Version: 7, Content: []byte(ignoredInterface)},
		oversizedPath: {
			Version: 7,
			Content: append(
				[]byte(ignoredInterface),
				make([]byte, generationInventoryMaximumFileBytes+1-int64(len(ignoredInterface)))...,
			),
		},
		smallPhysicalPath: {
			Version: 7,
		...
		largePhysicalPath: {
			Version: 7,
			Content: []byte(`// @import { NamedInterface } from "github.com/spice-framework/spice/annotation/modulith"

// @NamedInterface("largephysical")
package largephysical
`),
		},
	}
	for _, relative := range ignoredPaths {
		overlay[filepath.Join(root, filepath.FromSlash(relative))] = Document{
			Version: 7,
			Content: []byte(ignoredInterface),
		}
	}
	overlay[filepath.Join(
		...
		"ignored",
		"doc.go",
	)] = Document{Version: 7, Content: []byte(ignoredInterface)}
	if linkedPath != "" {
		overlay[linkedPath] = Document{Version: 7, Content: []byte(ignoredInterface)}
	}
	...
	if !slices.Equal(got, []string{
		"example.com/servicefixture/internal/processcontainment/largephysical",
...
func TestOrdinaryGenerationDiscoversKnownModuleNamedInterfaceOnce(t *testing.T) {
	root := writeGenerationModuleScopeFixture(t, false)
	writeServiceFixtureFile(t, root, "internal/processplatform/doc.go", `// @import { Module } from "github.com/spice-framework/spice/annotation/modulith"

// Package processplatform owns platform behavior.
// @Module(allowedDependencies=["example.com/servicefixture/internal/architectureproof::spi", "example.com/servicefixture/internal/processcontainment"])
package processplatform
`)
	writeServiceFixtureFile(t, root, "internal/runidentity/doc.go", `// @import { Module } from "github.com/spice-framework/spice/annotation/modulith"

// @Module(allowedDependencies=["example.com/servicefixture/internal/architectureproof::spi"])
package runidentity
`)
	writeServiceFixtureFile(t, root, "internal/architectureproof/spi/doc.go", `// @import { NamedInterface } from "github.com/spice-framework/spice/annotation/modulith"

// Package spi exposes architecture identity without joining the application graph.
// @NamedInterface("spi")
package spi
`)
	writeServiceFixtureFile(t, root, "internal/architectureproof/spi/foreign.go", `package spi

// @import { Application, Bean, Singleton } from "github.com/spice-framework/spice/annotation/core"
...
// @Bean
// @Singleton
func NewForeignService() *ForeignService { return &ForeignService{} }
`)

	interfacePattern := "example.com/servicefixture/internal/architectureproof/spi"
	interfaceLoads := 0
	service, err := New(Config{Loader: func(
		ctx context.Context,
		options load.Options,
		...
		if slices.Contains(patterns, interfacePattern) {
			interfaceLoads++
		}
		return load.Load(ctx, options, patterns...)
	}})
	...
	if diagnostics := result.Diagnostics().Items(); len(diagnostics) != 0 {
		t.Fatalf("known-module named-interface diagnostics = %#v", diagnostics)
	}
	var architectureModule Module
	for _, module := range result.ModuleGraph().Modules {
		if module.ID == "example.com/servicefixture/internal/architectureproof" {
			architectureModule = module
			break
		}
	}
	if !slices.Equal(architectureModule.NamedInterfaces, []NamedInterface{{
		Name:        "spi",
		PackagePath: interfacePattern,
	}}) {
		t.Fatalf("known-module named interfaces = %#v", architectureModule.NamedInterfaces)
	}
	graph := result.ModuleGraph()
	graph.Modules[0].NamedInterfaces = append(
		graph.Modules[0].NamedInterfaces,
		NamedInterface{Name: "mutated", PackagePath: "example.com/mutated"},
	)
	refreshed := result.ModuleGraph()
	for _, module := range refreshed.Modules {
		for _, named := range module.NamedInterfaces {
			if named.Name == "mutated" {
				t.Fatal("generation module graph returned mutable identity storage")
			}
		}
	}
	modelModule, found := result.ModuleModel().Owner(interfacePattern)
	if found || modelModule.ID != "" {
		t.Fatalf("target-local module model merged identity package = %#v, %t", modelModule, found)
	}
	if interfaceLoads != 2 {
		t.Fatalf("duplicate known identity interface loads = %d, want 2", interfaceLoads)
	}
	if targets := result.ApplicationModel().Targets(); len(targets) != 1 ||
		targets[0].Name != "ArchitectureProof" {
		t.Fatalf("identity inventory merged foreign applications = %#v", targets)
	}
	plan, found := result.GenerationPlan()
	if !found {
		t.Fatal("known-module named-interface generation plan was not produced")
	}
	for _, file := range plan.Files() {
		if strings.Contains(string(file.Content()), "ForeignService") ||
			strings.Contains(string(file.Content()), "ForeignApplication") {
			t.Fatal("identity inventory merged foreign providers into generation")
		}
	}
}

func TestGenerationNamedInterfaceOverlayErrorsAreSorted(t *testing.T) {
	root := t.TempDir()
	moduleRoot := filepath.Join(root, "module")
	for _, relative := range []string{"a", "z"} {
		if err := os.MkdirAll(filepath.Join(moduleRoot, relative), 0o750); err != nil {
			t.Fatal(err)
		}
	}
	moduleDocument := filepath.Join(moduleRoot, "doc.go")
	if err := os.WriteFile(moduleDocument, []byte("package module\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	module := modulith.Module{
		ID: "example.com/module",
		PhysicalPosition: token.Position{
			Filename: moduleDocument,
		},
	}
	overlay := map[string]Document{
		filepath.Join(moduleRoot, "z", "bad.go"): {Content: []byte("package")},
		filepath.Join(moduleRoot, "a", "bad.go"): {Content: []byte("package")},
	}
	var previous string
	for attempt := 0; attempt < 5; attempt++ {
		_, err := generationNamedInterfacePatterns(t.Context(), root, module, overlay)
		if err == nil || !strings.Contains(err.Error(), filepath.Join("a", "bad.go")) {
			t.Fatalf("sorted overlay parse error = %v", err)
		}
		if attempt == 0 {
			previous = err.Error()
		} else if err.Error() != previous {
			t.Fatalf("overlay parse error changed: %q != %q", err, previous)
		}
	}
}
