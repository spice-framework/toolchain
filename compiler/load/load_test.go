...
`,
	...
	if symbolByID(
		program.Symbols(),
		"example.com/overlaymain.spiceMain",
	) != nil {
		t.Fatalf(
			"analysis bridge leaked into user symbols: %v",
			symbolIDs(program.Symbols()),
		)
	}
...
func TestLoadOverlayReplacesOnlyOwnedGeneratedMainBridge(t *testing.T) {
	t.Parallel()
	source := `package main
...
`
	root := writeModule(t, map[string]string{
		"go.mod":  "module example.com/ownedbridge\n\ngo 1.26.0\n",
		"main.go": source,
		"zz_spice_bridge_gen.go": `//go:build !spice_generate

// Code generated by Spice. DO NOT EDIT.

package main

import "example.com/missing/generated"

func spiceMain(arguments []string) int {
	return generated.Main(arguments)
}
`,
	})
	mainPath := filepath.Join(root, "main.go")
	program, err := Load(context.Background(), Options{
		...
		Overlay: map[string][]byte{
			mainPath: []byte(source),
		},
	}, ".")
	if err != nil {
		t.Fatalf("Load(owned generated bridge) error = %v", err)
	}
	...
	if writeErr := os.WriteFile(
		...
		t.Fatalf("WriteFile(manual bridge) error = %v", writeErr)
	}
	if _, loadErr := Load(context.Background(), Options{
		Dir:                      root,
		AllowGeneratedMainBridge: true,
		BuildFlags:               []string{"-tags=spice_generate"},
		Overlay: map[string][]byte{
		...
		t.Fatal("Load(manual bridge) error = nil")
	}
}
...
`,
		"cmd/shop/zz_spice_bridge_gen.go": `//go:build !spice_generate
...
func TestOnlyGeneratedMainBridgeMessage(t *testing.T) {
	t.Parallel()
	if !onlyGeneratedMainBridgeMessage(
		"# example.com/app\nD:/Temp/gocommand/1-main.go:9:10: undefined: spiceMain",
	) {
		t.Fatal("exact generated bridge driver message was rejected")
	}
	for _, message := range []string{
		"",
		"exit status 1",
		"D:/main.go:9:10: undefined: missing",
		"D:/main.go:9:10: undefined: spiceMain\nD:/main.go:10:2: undefined: missing",
	} {
		if onlyGeneratedMainBridgeMessage(message) {
			t.Fatalf("unrelated driver message %q was accepted", message)
		}
	}
}
