...
func TestReleaseArtifactsVerifyExtractExecuteAndCleanInOrder(t *testing.T) {
	...
	gate := verifier{
		...
		executeStreams: func(
			...
			return []byte("spice 0.1.0-preview.6 (" + strings.Repeat("a", 40) + ")\n"), nil, nil
		},
	}
...
func TestReleaseArtifactsFailClosed(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			var calls []string
			set := &fakeReleaseSubjectSet{version: "v0.1.0-preview.6", commit: strings.Repeat("b", 40), calls: &calls}
			gate := verifier{
				output:         io.Discard,
				...
				executeStreams: func(context.Context, string, map[string]string, string, ...string) ([]byte, []byte, error) {
					return []byte("spice 0.1.0-preview.6 (" + strings.Repeat("b", 40) + ")\n"), nil, nil
				},
			}
