...
func TestArchiveValidationRejectsZipDrift(t *testing.T) {
	...
	target := releaseTarget{
		GOOS: "windows", GOARCH: "amd64", Archive: "toolchain_0.1.0-preview.8_windows_amd64.zip",
...
func TestArchiveValidationRejectsPathModePayloadAndMembershipDrift(t *testing.T) {
	...
	target := releaseTarget{
		GOOS: "linux", GOARCH: "amd64", Archive: "toolchain_0.1.0-preview.8_linux_amd64.tar.gz",
...
func fixtureMetadata(t *testing.T, directory string) releaseMetadata {
	...
	sbomContent := []byte(`{"spdxVersion":"SPDX-2.3","dataLicense":"CC0-1.0","SPDXID":"SPDXRef-DOCUMENT","name":"toolchain v0.1.0-preview.8","documentNamespace":"https://github.com/spice-framework/toolchain/releases/v0.1.0-preview.8/spdx/test","packages":[{"name":"github.com/spice-framework/toolchain","versionInfo":"v0.1.0-preview.8"}]}` + "\n")
