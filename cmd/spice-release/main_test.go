...
func TestRunRejectsInvalidInvocationBeforeBuilding(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "stale preview.3 generator version",
			arguments: []string{
...
func TestValidateReleaseIntentAllowsCanonicalPrerelease(t *testing.T) {
	t.Parallel()
	if err := validateReleaseIntent("v0.1.0-preview.5", "private-key", false); err != nil {
...
func TestRunBuildsHostRehearsal(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(output, "spice_0.1.0-preview.5_source.tar.gz")); err != nil {
		t.Fatal(err)
	}
...
`,
	...
	runGit(t, root, "tag", "v0.1.0-preview.5")
	...
	for _, name := range []string{
		...
		"checksums.txt.pem",
		"checksums.txt.sig",
		"spice_0.1.0-preview.5_source.tar.gz",
	} {
