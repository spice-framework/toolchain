...
func TestRunPolicyCheckAuthorizesExactComparablePolicies(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "Toolchain distribution", repository: "toolchain", version: "v0.1.0-preview.6", profile: "go-distribution-v1"},
...
func TestRunPolicyCheckEmitsCanonicalToolchainPreviewSixTuple(t *testing.T) {
	t.Parallel()
	const (
		want       = "go-distribution-v1\ttoolchain\tgithub.com/spice-framework/toolchain\tv0.1.0-preview.6\n"
		wantSHA256 = "fde8f5c596008bb859a9776650849fbc19562cd7f2d822a05596a0e3e26b5b1d"
	)
	var stdout, stderr bytes.Buffer
	code := run(context.Background(), []string{
		"policy-check",
	...
	if code != 0 || stdout.String() != want ||
		...
		t.Fatalf(
			"run(Toolchain preview.6 policy-check) = %d, stdout %q, sha256 %s, stderr %q",
			code,
			stdout.String(),
